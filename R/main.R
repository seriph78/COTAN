...
#' @export
#'
#' @examples obj = initRaw(obj,GEO="GSE95315" ,sc.method="10X",cond = "mouse dentate gyrus P0 subclustering of cl.2 - 0,1,2")
...
#' clean.sqrt
#'
#' @param object It is a COTAN object
#' @param cells matrix of 0/1
#'
#' @return
#' @export
...
#' obj = ttm$object
#' ttm$pca.cell.2}
setGeneric("clean.sqrt", function(object, cells) standardGeneric("clean.sqrt"))
setMethod("clean.sqrt","scCOTAN",
          function(object) {
              cells = cells[rowSums(cells)> round((length(colnames(raw))/1000*1), digits = 0),] # 3 per mille delle cellule
              object@raw = object@raw[rownames(cells), colnames(cells)]
              #fwrite(cells,paste(out_dir,"cells_",t,".csv", sep = ""))
              #save(n_it,file = paste(out_dir,"n_it_",t, sep = ""))
              list1 = fun_sqrt(object)
              # list1 = fun_iter_no_nu(cells = cells, raw = raw)
              gc()
              ...
              output = list("cl1"=cl1,"cl2"=cl2,"pca.cell.2"=pca.cell.2,"object"=object, "mu_estimator"=mu_estimator, "D"=D, "pca_cells"=pca_cells, "nu.data"=list1$nu.data,"lamba.data"= list1$lamba.data)
              return(output)
          }
