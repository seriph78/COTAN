...
checkClusterUniformity <- function(objCOTAN, clName, cells,
  ...
  if (saveObj) {
    ...
    genesToLabel = head(rownames(GDI_data[order(GDI_data[["GDI"]], decreasing = T), ]), n = 10)
    plot(GDIPlot(objCOTAN, GDI.df = GDI_data, genes = list("top 10 GDI genes" = genesToLabel)))
  ...
  clusterIsUniform <- (nrow(GDI_data[GDI_data[["GDI"]] >= 1.5, ]) <=
                         0.01 * nrow(GDI_data))
