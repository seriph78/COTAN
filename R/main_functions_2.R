          ...
          function(data.seurat,data.raw,out_dir, cond, cores){
            ...
            for (cl in c(0:tot.clusters)) { #0
              ...
              if (n_cells <= 10) {
                ...
                #xdensityGDI <- ggplot(GDI_data_wt1, aes(sum.raw.norm)) + 
                #   geom_density(alpha=.5, fill = "#8491B4B2", colour ="#8491B4B2" ) #+
                
                
                #ydensityGDI <- ggplot(GDI_data_wt1, aes(GDI)) + 
                #   geom_density(alpha=.5, fill="#00A087FF", colour= "#00A087FF") +coord_flip() 
                
                
                #GDI_plot_wt1= xdensityGDI + plot_spacer() + GDI_plot_wt1 + ydensityGDI + 
