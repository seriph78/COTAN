...
setMethod(
  ...
  function(object) {
    ...
    new_estimator_yes_yes_n <- mat2vec_rfast(est$expectedYY)
    new_estimator_yes_yes_n$values[new_estimator_yes_yes_n$values < 1] <- 1
    new_estimator_yes_no_n  <- mat2vec_rfast(est$expectedYN)
    new_estimator_yes_no_n$values[ new_estimator_yes_no_n$values  < 1] <- 1
    new_estimator_no_no_n   <- mat2vec_rfast(est$expectedNN)
    new_estimator_no_no_n$values[  new_estimator_no_no_n$values   < 1] <- 1
    new_estimator_no_yes_n  <- mat2vec_rfast(est$expectedNY)
    ...
    coex_n <- (yes_yes_n$values - mat2vec_rfast(as.matrix(est$expectedYY))$values)

    rm(est)

    sum.for.div_n <- (  1 / new_estimator_yes_yes_n$values + 1 / new_estimator_no_no_n$values
                      + 1 / new_estimator_yes_no_n$values  + 1 / new_estimator_no_yes_n$values)
