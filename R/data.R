...
#'
#' @description A brief explanation of how to install the torch package on WLS2
#'   Linux. Naturally it makes a difference whether one wants to install support
#'   only for the `CPU` or also have the eventual system `GPU` at the ready!
#'
...
#'   libraries are installed, like BLAS and LAPACK and/or MKL if your `CPU` is
#'   from Intel. Otherwise torch will be stuck at using a single core for all
#'   computations.
#'
...
#'
#'   Currently only `cuda` 11.7 and 11.8 are supported, but check the `torch`
...
#'   main versions of `cuda` contemporaneously on the system: that is one can
#'   have installed 11.8 and a 12.4 `cuda` versions at the same time.
#'
#'   Below a link to install `cuda` 11.8 for `WSL2` Linux Windows sub-system is
#'   given: use a local installer to be sure the wanted `cuda` version is being
#'   installed, and not the latest one: [cuda 11.8 for
