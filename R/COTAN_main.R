...
setMethod(
  ...
  function(object, GEO, sc.method, cond) {
    ...
    if ((!attr(object@raw, "class")[1] == "dgCMatrix") |
      ...
      object@raw <- methods::as(
        as.matrix(object@raw),
        "sparseMatrix"
      )
    }
...
setMethod(
  ...
  function(object) {
    ...
    D <- data.frame(
      "means" = rowMeans(C[2:length(colnames(C))]),
      "n" = NA
    )
    D <- D[D$means > 0, ]
    ...
    pca.cell.1 <- ggplot(
      subset(pca_cells, groups == "A"),
      aes(x = PC1, y = PC2, colour = groups)
    ) +
      geom_point(alpha = 0.5, size = 3)

    pca.cell.2 <- pca.cell.1 + geom_point(
      data = subset(
        pca_cells,
        groups != "A"
      ), aes(x = PC1, y = PC2, colour = groups),
    ...
    while (p <= length(rownames(mu_estimator))) {
      if ((p + 200) <= length(rownames(mu_estimator))) {
        tot1 <- parallel::mclapply(rownames(mu_estimator)[p:(p + 200)],
          fun_my_opt,
          ce.mat = cells, mu_est = mu_estimator,
          mc.cores = cores
        )
      } else {
        print("Final trance!")
        tot1 <- parallel::mclapply(rownames(mu_estimator)[p:length(rownames(mu_estimator))],
          fun_my_opt,
          ce.mat = cells, mu_est = mu_estimator, mc.cores = cores
        )
      }
      tot <- append(tot, tot1)
      p <- p + 200 + 1
