...
#' @param mt T or F if the mitochondrial genes should be kept or removed (default F)
#' @param mt_prefix Prefix for the mitochondrial genes
            ...
            while (!identical(cl1_not_mergiable,cl1_not_mergiable_old)) {
              ...
              while (p < length(id)) {
                ...
                merged.obj <- automatic.COTAN.object.creation(df = as.data.frame(mat),out_dir = out_dir ,GEO = GEO
                                                              ,sc.method=sc.method,cond = cond.merge,cores = cores,mt = mt,mt_prefix = mt_prefix  
                )
