...
and we can plot the distribution of the numbef of detected genes in each cell,
usefull to identify possible doublets
...
```{r Scatter plot: Cells size vs Number of detected genes - II}
plot(scatterPlot(obj, condName = "origin"))
```
...
The above plot shows a sub-group of cells `B` that is samewhat distinct from
the other cells
...
Checking which genes are typica of the new `B` group, shows that there is no
need of further culling

```{r Show most expressed genes of B group - II}
plot(genesPlot)
```
...
```{r Save COTAN obj to file, eval=FALSE, include=TRUE}
...
```{r COTAN object construction for no cleaning case, eval=FALSE, include=TRUE}
obj2 <-
