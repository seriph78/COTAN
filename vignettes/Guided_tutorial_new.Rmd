...
```{r}
obj <- clean(obj)
list[pcaCellsPlot, genesPlot, UDEPlot, nuPlot] <- cleanPlots(obj)
...
obj <- automaticCOTANObjectCreation(
  ...
  saveObj = TRUE, outDir = out_dir, cores = 10)
```
...
```{r}
clusters <- cellsUniformClustering(obj, cores = 10, saveObj = TRUE, outDir = out_dir)
...
obj <- addClusterizationCoex(obj, clName = "test", coexDF = coex)
```
...
```{r}
clusterData <- getClusterData(obj)[["clusters"]]

######## This is the best: cosine dissimilarity
tree <- hclust(cosineDissimilarity(clusterData), method = "ward.D2")
