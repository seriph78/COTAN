...
Using the cluster distance, based on the whole transcriptome, we can check if any of the closest clusters can together form a still uniform cluster. We will keep this clusterization for the forther analysis.
...
#plot_heatmap(df_genes = list.genes, sets = c(1:3), conditions = c("test"), dir = outDir)
```

We can also plot a general heatmap of coex values based on some markers like the following one.

```{r}
plot_general.heatmap(prim.markers = AA, condition = "test",dir = outDir, p_value = 0.001)
```

```{r}
plot_general.heatmap(prim.markers = AA ,markers.list = BB ,symmetric = FALSE, condition = "test",dir = outDir, p_value = 0.001)
```
...
```{r}
list[observedCT, expectedCT] <- contingencyTables(obj, g1 = "EIF3F", g2 = "SLC37A1")
print("Observed CT")
observedCT
print("Expected CT")
expectedCT
