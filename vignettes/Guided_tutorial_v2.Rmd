...
sampleDataset <- read.csv(dataSetFile, sep = "\t", row.names = 1L)
```

Define a directory where the output will be stored.

```{r Setup log and check for GPU}
outDir <- dataDir
...
```{r Drop the mitochondrial genes}
genesToRem <- getGenes(obj)[grep("^Mt", getGenes(obj))]
obj <- dropGenesCells(obj, genes = genesToRem)

cellsToRem <- getCells(obj)[getCellsSize(obj) == 0L]
obj <- dropGenesCells(obj, cells = cellsToRem)
```
...
```{r Heatmap plot of coex for selected genes}
...
```{r Heatmap plot of coex given primary marker genes, eval=TRUE, include=TRUE}
invisible(
  genesHeatmapPlot(
    obj,
...
```{r Heatmap plot of coex given two genes sets, eval=FALSE, include=TRUE}
invisible(
  genesHeatmapPlot(
    obj,
...
```{r Establish genes clusters, eval=TRUE, include=TRUE}
layersGenes <- list(
  "L1"   = c("Reln",   "Lhx5"),
  "L2/3" = c("Satb2",  "Cux1"),
  "L4"   = c("Rorb",   "Sox5"),
...
```{r Genes tree plot, eval=TRUE, include=TRUE}
plot(treePlot)
```

```{r Genes UMAP plot, eval=TRUE, include=TRUE}
...
```{r Extract clusterization summary, eval=TRUE, include=TRUE}
...
```{r Summary as plot, eval=TRUE, include=TRUE}
plot(summaryPlot)
```
...
```{r More complete clusterization data visualization, eval=TRUE, include=TRUE}
...
```{r Calculate and store DEA using clusterizations, eval=TRUE, include=TRUE}
...
```{r  Clusterizations data visualization, eval=TRUE, include=TRUE}
...
```{r Clusterization UMAP plot using Dimensionality Reduction}
c(umapPlot, cellsRDM) %<-%
  cellsUMAPPlot(
    obj,
    clName = "merge2",
