...
satisfies the null hypothesys of the `COTAN` model:
the genes expression is not depdendent on the cell in consideration.
...
```{r}
deaDfs <- DEAOnClusters(obj, clusters = fineClusters)
...
```{r}
fineUMAPPlot <- UMAPPlot(deaDfs[["coex"]], title = "Fine Cluster UMAP Plot")
...
```{r}
mergedList <- mergeUniformCellsClusters(obj, GDIThreshold = 1.4, cores = 10,
                                        saveObj = TRUE, outDir = outDir)
...
```{r}
mergedUMAPPlot <- UMAPPlot(mergedList[["coexDF"]],
