...
cells_to_rem <- names(numExprGenes)[numExprGenes > 3000L]
obj <- dropGenesCells(obj, cells = cells_to_rem)
...
```{r}
obj <- estimateDispersionBisection(obj, cores = 10L)
```
...
obj2 <- automaticCOTANObjectCreation(
  ...
  sequencingMethod = "Drop_seq",
  sampleCondition = cond,
  saveObj = TRUE, outDir = outDir, cores = 10L)
```
...
```{r eval=FALSE, include=TRUE}
fineClusters <- cellsUniformClustering(obj, GDIThreshold = 1.4, cores = 10L,
