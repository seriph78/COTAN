...
```{r}
obj <- proceedToCoex(obj, calcCoex = TRUE, cores = 6L,
...
obj2 <- automaticCOTANObjectCreation(
  ...
  sequencingMethod = "Drop_seq",
  sampleCondition = cond,
  calcCoex = TRUE, saveObj = TRUE, outDir = outDir, cores = 6L)
```
...
```{r eval=FALSE, include=TRUE}
coexDF <- DEAOnClusters(obj, clusters = fineClusters, cores = 6L)
...
c(mergedClusters, coexDF) %<-%
  mergeUniformCellsClusters(obj, GDIThreshold = 1.43, cores = 6L,
                            saveObj = TRUE, outDir = outDir)
