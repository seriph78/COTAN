...
```{r eval=TRUE, include=TRUE}
genesClusters <- set_names(pcaGenesClDF[["hclust"]], rownames(pcaGenesClDF))
numericCols <- sapply(pcaGenesClDF, is.numeric)

genesUmapPl <- UMAPPlot(pcaGenesClDF[, numericCols, drop = FALSE],
                        clusters = genesClusters,
...
```{r}
c(umapPlot, cellsPCA) %<-% cellsUMAPPlot(obj, clName = "merge", colors = NULL,
                                         method = "AdjZeroOne", genesSel = NULL)
