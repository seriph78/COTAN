...
#data = as.data.frame(fread(system.file("extdata", "E175_only_cortical_cells.txt.gz", 
                                       #package = "COTAN", mustWork = TRUE),sep = "\t"))
data = as.data.frame(fread(system.file("extdata", "ercc.csv", 
                                       package = "COTAN", mustWork = TRUE),sep = ","))
...
Next we can define some gene sets (in this case three) that we want to specifically lable in the GDI plot.
...
plot_heatmap(df_genes = list.genes,sets = c(1:3),conditions = "ERCC",dir = "../inst/extdata/")
```

We can also plot a general heatmap of coex values based on some markers like the following one.

```{r}
plot_general.heatmap(prim.markers = c("ERCC-00014","ERCC-00019"), condition = "ERCC",dir = "../inst/extdata/", p_value = 0.05,)
```
