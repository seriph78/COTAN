...
look nicer in plots and lists, for example to ensure all labels have
the **same** length.

Another improvement is to reorder the labels of the *clusterization* so that
*near* clusters have *near* labels
...
```{r cl-data-visualization_found-markers, fig.height=8, fig.width=20}
c(mergeHeatmap, ..) %<-%
  clustersMarkersHeatmapPlot(
    obj,
...
In the case of only two clusters, over-expressed genes in a cluster correspond
exactly to under-expressed genes in the other,
so one can just look at the former 
